...
- [ ] 编译/Lint 通过后，运行实际测试：`execute("npm test -- --no-cache")`（Web 项目）或 `execute("npm test")`（App 项目）。App 项目还需跑 Kotlin 测试：`execute("cd android && ./gradlew.bat testDebugUnitTest")`。如果项目没配测试则跳过，不报错
