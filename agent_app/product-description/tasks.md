...
> 状态：0.2.1-FE + 0.2.2-FE 已验证通过 ✅，剩余 0.2.3-FE（可视化测试界面 + 本地存音频）、0.2.4-BE（音频流接收）、0.2.5-BE（采集效果验证）
...
- [ ] 0.2.3-FE：**可视化测试界面**——在原 PoC 基础上加一个简单的测试面板：
  - 📍 采集状态指示灯（绿=采集中 / 红=已停止 / 黄=权限等待）
  - ⏱️ 实时计时器（已采集 X 分 X 秒）
  - 📊 简易音量指示（当前音频电平，确认有数据流入）
  - 💾 音频本地存储——把采集到的 PCM 流存成文件（如 `.pcm` 或 `.wav`），方便后续给 BE 做格式验证和 ASR 测试
  - 音频格式：FE/BE 自行对齐（建议 16kHz / 16bit / 单声道 PCM，通用优先）

#### BE 任务清单
- [ ] 0.2.4-BE：接收前端音频流——搭建音频推流接收端点（WebSocket 或 HTTP 分块），验证收到的 PCM 格式（采样率/位深/声道数）是否符合 ASR（腾讯云）要求
- [ ] 0.2.5-BE：采集效果验证——拿到 FE 本地存的音频文件后，验证：① 面试官声音清晰可辨 ② 无异常截断/静音段 ③ 格式可直接喂腾讯云 ASR

#### 验收标准
- [x] 手机端能通过系统音频流实时采集线上面试音频
- [x] 面试官声音能被清晰采集（系统内部音频通道，无物理距离概念）
- [ ] 可视化测试界面：状态灯 + 计时器 + 音量指示正常工作
- [ ] 音频文件成功保存到本地，可导出
