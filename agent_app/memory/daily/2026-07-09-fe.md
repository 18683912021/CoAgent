...
项目代码目前不在磁盘上了——`workspace/fe/` 是空的，`poc-audio-capture/` 目录也不存在。  不过根据之前的调试记录，这两个 bug 的根因已经定位过：  **Bug 1「开始采集 → 白屏」**：很可能是 JS 层未捕获异常导致 React 组件树崩溃。之前修过 `screen.tsx` 的运行时权限申请（`PermissionsAndroid.request(RECORD_AUDIO)`），但如果原生模块 `start()` 抛出未 handle 的异常（比如 AudioRecord 初始化失败）、JS 侧没有 try/catch 或 ErrorBound
